...
- **非 workspace メンバー**:
  ...
  - `crates/j-law-java/`（Gradle Wrapper で管理、JNI 経由で `j-law-c-ffi` にリンク）
...
| Java | `crates/j-law-java/src/main/java/**` + `src/main/c/**` + `build.gradle` | JNI で `j-law-c-ffi` をラップ |
...
### Java JNI バインディングのネイティブライブラリ解決

Java バインディングは JNI 共有ライブラリを以下の順で探索します:

1. `-Djlaw.native.lib=/path/to/libjlaw_jni.{so|dylib|dll}`
2. `JLAW_JAVA_NATIVE_LIB`
3. JAR 同梱の `META-INF/jlaw/native/<os>/<arch>/`

ローカル開発では `./gradlew test` 実行時に `cargo build -p j-law-c-ffi --target <jni向けtarget>` が自動実行され、その成果物をリンクして JNI ライブラリを生成します。
