...
- **目的**: 日本の法令・告示・省令が定める各種計算のコード化を検証する PoC / コンセプト実装
- **アーキテクチャ**: ドメイン単位で法令を追加できる設計
- **Rust エディション**: 2021
- **ステータス**: `v0.0.1` の alpha。公開ドキュメントや package metadata で「法的正確性を保証」「本番利用可能」と断定しないこと
...
- **非 workspace メンバー**:
  ...
  - `crates/j-law-ruby/`（`gemspec` で管理、`ffi` 経由で `j-law-c-ffi` にリンク）
...
| ドメイン | 法的根拠 | ディレクトリ |
|---|---|---|
| 消費税（`consumption_tax`） | 消費税法 第29条 | `domains/consumption_tax/` |
| 不動産（`real_estate`） | 宅建業法 第46条 / 国交省告示 | `domains/real_estate/` |
| 所得税（`income_tax`） | 所得税法 第89条 / 復興財源確保法 第13条 | `domains/income_tax/` |
| 印紙税（`stamp_tax`） | 印紙税法 別表第一 | `domains/stamp_tax/` |
| 源泉徴収（`withholding_tax`） | 所得税法 第204条第1項 | `domains/withholding_tax/` |
...
新しい法令ドメインは、少なくとも以下の基本構成を持つこと。必要に応じて `assessment.rs` や `deduction.rs` などの補助モジュールを追加してよいです。
...
5. **主要 calculate 関数**: `Result<CalculationResult, JLawError>` を返す主計算関数
6. **CalculationResult**: `breakdown` を含む結果型
7. **公式計算例テスト**: 省庁や公的機関が公表している計算例を可能な限りテスト化
