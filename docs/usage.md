...
JNI を使って `j-law-c-ffi` の C ABI を呼び出す Java / JVM バインディングです。公開 API は Java 8 互換で、`LocalDate` と `long` を使います。
...
System.out.println(fee.getTotalWithoutTax()); // 210000
System.out.println(fee.getTotalWithTax());    // 231000

System.out.println(
    WithholdingTax.calcWithholdingTax(
        1_500_000L,
        0L,
...
1. `-Djlaw.native.lib=/absolute/path/to/libjlaw_jni.{so|dylib|dll}`
